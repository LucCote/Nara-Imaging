...
# returns 2 maps of the src image pixel x,y coords to global latitude and longitude coords
def load_coords(src):
  band1 = src.read(1)
...
# read !nara (long,lat) coords from given csv file
def load_points(file):
  ...
  with open(file) as csvfile:
      ...
      for row in reader:
          points.append((float(row[9]),float(row[8])))
  return points

# convert set of pixel x,y coordinates into latitude and longitude coordinate
...
# return if a point is within a very small distance of a shape
def point_in_shape(shape, point):
  pointS = Point(point)
  polygon = Polygon(shape)
  if polygon.contains(pointS):
    return True
  return polygon.exterior.distance(pointS) < 1e-4 # slight margin of error/inaccuracy

# generate true labels for each found contour given the truth file
# use this to also generate true/false positives and negatives.
def get_metrics(src, contour_shapes, contour_tags, truth_file):
  ...
  for i in range(len(contour_shapes)):
     ...
     for point in true_loc: # check if any labelled !nara in contour
        if point_in_shape(coords_shape, point):
          true_tags[i] = True
          break
     if tag and true_tags[i]: # true pos
      true_pos += 1
      # find contour values and append to proper array
      image_data=src.read()
      image = src.read(1)
     ...
     elif tag and not true_tags[i]: # false pos
      false_pos += 1
      # find contour values and append to proper array
      image_data=src.read()
      image = src.read(1)
      ...
      metric_labels[i] = 1
     elif not tag and not true_tags[i]: # true negative
      true_neg += 1
      # find contour values and append to proper array
      image_data=src.read()
      image = src.read(1)
      ...
      metric_labels[i] = 3
  # write out contour data for each metric
...
# just used for testing when running script standalone
